<?php
    ...
    if (isset($_GET['id'])){
        $ID = $_GET['id'];
        $query = $conn->query("SELECT * FROM tbgrup WHERE id = '".$ID ."'");
...
<div class="container-fluid">
            ...
            if (isset($_POST['edit'])){
                ...
                $rubahdata = $conn->query("UPDATE tbgrup SET name = '".$name."', komentar = '".$comment."', valid = '".$sah."', tglupdate = NOW() WHERE id = '".$_GET['id']."'");

                if ($rubahdata){
                    echo "<script>alert('Data Kelompok Berhasil di Edit!');window.location.href='index.php?agen=admin&view=group';</script>";
                }else{
                    echo "<script>alert('Data Kelompok Gagal di Edit!');</script>";
                }
            }
        }
        ?>
